...
## v1.6.4（59）｜2026-09-16

### 变更
- **吉祥物定稿「饭团仔」（用户选定参考图）**：弃用手绘矢量两轮迭代，
  直接采用用户选定的参考图——蓬松花边身体 + 眯眯眼 + 张嘴吐舌笑 + 双杠腮红 +
  大海苔 + 小手小脚；应用内 5 处（首页/引导×2/分享导入/池决策）改位图显示
  （`drawable-nodpi/mascot_onigiri.png`，圆角贴纸风）
- **启动器图标**：参考图裁白边后按 76% 居中于奶油底合成前景 PNG
  （吉祥物约占 66%，适配圆形遮罩）；单色层移除（系统主题化回退普通图标）；
  v1.6.1 的两个矢量 drawable 一并删除

## v1.6.3（58）｜2026-09-11

### 变更
- **饭团仔改椭圆形**：身体由圆顶三角改为整蛋形椭圆团子（无棱角），
  海苔改为收在蛋形内的小围裙（底部露出一点米饭）；应用内吉祥物与启动器图标同步更新

## v1.6.2（57）｜2026-09-11

### 变更
- **饭团仔去尖角**：饭团身体由尖顶三角改为圆顶三角团子——顶部整弧、两侧外鼓、
  底角圆润，整体更圆滚可爱；应用内吉祥物与启动器图标（前景/单色层）同步更新
